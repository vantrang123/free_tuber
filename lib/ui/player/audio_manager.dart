...
class AudioManager {
  ...
  get isPlaying => _player?.playing ?? false;

  get currentPlaybackPosition => _player?.position;

  Video? currentVideo;

  Future<void> addMediaItem(String audioUrl, dynamic video) async {
    try {
      currentVideo = null;
      _player?.stop();
  ...
  void onPositionChange(Function(Duration) onChange) {
    _player?.positionStream.listen((event) {
      if (event.inMilliseconds != 0) currentVideo?.lastDuration = event;
      onChange(event);
    });
  }
